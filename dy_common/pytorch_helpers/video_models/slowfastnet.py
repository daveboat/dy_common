"""
SlowFast video classification networks, from https://arxiv.org/abs/1812.03982

Code from https://github.com/r1ch88/SlowFastNetworks with very minor modifications

Implementation details from the paper:
    - For all models they tried, the ratio between the slow stride and the fast stride was alpha=8.
    - They experiment with two strides and two clip lengths:
        - slow stride 16, fast stride 2, 64 frames total (so slow network gets 4 frames, fast network gets 32 frames) -- they used resnet50 for this
        - slow stride 8, fast stride 1, 64 frames total (so slow network gets 8 frames, fast network gets 64 frames) -- they used resnet50 and resnet101 for this
        - slow stride 8, fast stride 1, 128 frames total (so slow network gets 16 frames, fast network gets 128 frames) -- they used resnet101 for this
    - During training, they randomly sample the number of frames needed from the full-length video, scale the video so
    that its short side is randomly chosen from [256, 320], do a random 224x224 crop, and randomly horizontal flip
    - During inference, they uniformly sample 10 clips from the full-length video, rescale the frames so that the
    shorter side is 256 pixels, then take three 256x256 crops uniformly across the video frames, for a total of 30
    inference samples. The softmax scores of all 30 are then averaged for prediction.

The slow part of the network returns a feature length of 2048, and the fast part of the network returns a feature length
of 256. These are concatenated to a total feature length of 2304.
"""
...
class SlowFast(nn.Module):
    ...
    def forward(self, input):
        ...
        slow = self.SlowPath(input[:, :, ::self.stride * 8, :, :], lateral)
...
if __name__ == "__main__":
    num_classes = 101
    input_tensor = torch.randn((2, 3, 128, 224, 224))
    model = resnet50(class_num=num_classes, stride=1)
