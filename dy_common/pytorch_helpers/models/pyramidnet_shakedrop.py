"""
Pytorch implementation of PyramidNet + ShakeDrop regularization, from github.com/Dragonsson/SakeDrop-Pytorch

PyramidNet: https://arxiv.org/abs/1610.02915
ShakeDrop: https://arxiv.org/abs/1802.02375
"""
        ...
        if residual_channel != shortcut_channel:
            padding = torch.autograd.Variable(
                torch.cuda.FloatTensor(batch_size, residual_channel - shortcut_channel, featuremap_size[0],
                                       featuremap_size[1]).fill_(0) if x.is_cuda else torch.FloatTensor(batch_size, residual_channel - shortcut_channel, featuremap_size[0],
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        if residual_channel != shortcut_channel:
            padding = torch.autograd.Variable(torch.cuda.FloatTensor(batch_size, residual_channel - shortcut_channel, featuremap_size[0],
                                       featuremap_size[1]).fill_(0) if x.is_cuda else torch.FloatTensor(batch_size, residual_channel - shortcut_channel, featuremap_size[0],
                                       featuremap_size[1]).fill_(0))
            out += torch.cat((shortcut, padding), 1)
        else:
...
class PyramidNet_ShakeDrop(nn.Module):
    ...
    def __init__(self, depth, alpha, num_classes, bottleneck=False, return_feat=False):
        ...
        # self.p_drop is initialized to a geometric sequence,
        # refer to equation (5) in section II of the paper
        self.u_idx = 0
        all_depth = sum(layers[depth])
        self.p_drop = [0.5/all_depth * (i + 1) for i in range(all_depth)]  # note that the 1-p is implemented in the shakedrop module
...
if __name__ == '__main__':
    ...
    model = PyramidNet_ShakeDrop(depth=200, alpha=200, num_classes=6)
    model = model.to(torch.device('cuda'))
    model.train()
    out = model(inp)
    print(out.size())
    torch.sum(out, dim=-1).backward()
